...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        // BUGFIX: Show a persistent inline error on the login card instead of a toast.
        // The toast disappears when signOut() triggers onAuthStateChanged(null) → showView('login-ui'),
        // but the inline #login-error-msg div persists because showView does not touch it.
        if (!user.email.endsWith('@neu.edu.ph')) {
            ...
            // Reset login button state before signing out
        ...
        // Clear any previous login error on successful NEU email
        const errEl = document.getElementById('login-error-msg');
        ...
        currentUser = null;
        showView('login-ui');
        updateLibraryStatus(); // refresh status badge on login screen
    }
});
...
async function checkActiveLog(uid) {
    toggleLoading(true);
    try {
        // Two equality where clauses — no orderBy, so no composite index needed
...
async function performLogout() {
    // Clear every timer — countdownInterval is now module-level so this always works
...
document.getElementById('saveProfileBtn').onclick = async () => {
    ...
    try {
        await setDoc(doc(db, 'users', auth.currentUser.uid), {
            ...
            role:               'user',   // set to 'admin' ONLY via Firebase console — never self-declared
            isBlocked:          false,
            createdAt:          serverTimestamp()
        });
        showToast('Success', 'Profile setup complete! Welcome to the NEU Library.', 'success');
        await checkActiveLog(auth.currentUser.uid);
    } catch (error) {
